...
## 背景
我们管理台为了更好的服务用户，引导用户操作，所以给每个模块下面都准备了几个关于该模块的常见问题，用户点击这几个常见问题，就可以快捷的唤醒管理台的AI助手，然后AI助手会根据用户的问题，自动回复。

## 问题
我们接到测试人员的反馈，他们在测试过程中，发现有的时候AI助手会重复提问，就是点击某一个常见问题，但是，会向AI助手提出两个不同的问题，其中一个问题是用户点击的那个问题，即用户想要咨询的问题，还有一个问题不知道从何而来。

## 排查
由于负责这个项目的前端开发人员刚好离职了，领导就安排我去跟进这个问题。因为我们只是提供了一个AI助理的组件库给到各个业务侧，所以我确认是不是业务多次调用了我们暴露的接口，对埋点数据的查看，发现业务侧在用户点击的时候，只会调用一次接口，那基本可以确定是我们组件内部出了问题。
然后，我通过在控制台sourceMap断点，并且查看调用栈，最终定位了问题。

## 原因
通过查看调用栈发现，业务侧首次调用AI助理组件的时候，等到历史消息返回时，立即调用AI助理组件提供的提问方法，但是此时，由于组件并没有渲染，所以，拿不到组件暴露的方法。所以上一个开发人员是维护了一个事件队列。
当用户首次点击某一问题时，他会将这个问题放到队列中，当组件渲染完成后，会从队列中拿出问题，然后调用组件提供的提问方法。并且为了方便，在组件内部会将提问方法挂载到window上，这样业务侧就可以直接调用。
所以，组件内部整体逻辑是，先判断window上是否有提问的方法，如果没有，就往事件队列中塞入问题，如果有，就直接调用。等到组件渲染完成后，会从队列中拿出问题，然后调用组件提供的提问方法。

问题出在，后面用户点击了别的问题，会调用window上已经挂载了的方法，此时会发送提问，但是事件队列中的那个提问一直都在，没有删除，所以，组件渲染的时候，还是会从队列中拿出问题，然后调用组件提供的提问方法。这样就多出了一个提问。

## 解决方案
找到了问题的原因，我们只需要在组件渲染完成后，清空事件队列即可。


# 直播数据支持导出

## 背景
我们有一个需求，就是支持用户自定义表格头部字段以及头部字段的顺序。具体是，弹出一个面板，面板中有若个个字段，然后用户可以选择某些关注的指标，并且调整顺序，保存后，即可生成相应的表格。

## 问题
用户保存后，我们会保存一份给后端，另外会向父组件(以及将面板封装成了组件)传递用户的选择，我们发现新增或者删除一个指标，表格是可以按照预期渲染的，但是如果仅仅是调整顺序，即使数据已经发生了更改，但是ui并没有变化，即表格的头部没有做出相应的调整。

## 排查
首先我们想的是可能是数据更改和视图更改的不同步导致的，因为vue是这样的一个特性的，但是新增和删除是可以正常的，其实就应该不是这个问题，但是我们还是通过给一个v-if，然后在用户保存数据后，启动一个定时器，在定时器的回调中再改变v-if的值，这样果然没有效果。后面我们发现有可能是diff算法导致的，因为diff算法会尽可能的复用元素，所以我通过给table组件增加一个key，初始值为0，然后每次用户更改时，通过自增key值迫使表格强制重新渲染，最终解决这个问题。
后面想了一下，其实通过jsx也可以实现，因为jsx我们是写在一个render函数中，每当数据发现变化是，render函数都会重新执行一下，也就是重新基于新的数据渲染。

# 广告投放数据看板

## 背景
我们有一个echarts开发的折线图，它是根据筛选条件来请求后端数据生成的，x轴数据，图例，y轴的数据都是依据后端数据动态生成的。

## 问题
当用户快速重复点击时，控制台会不断发送请求，并且echart内部会抛出错误，它认为我们的legend，就是图例重复了，所以导致echart没办法重新绘制

## 解决方案

针对这个问题，当时我这边时提出了三个方案：

**方案一：** 点击筛选按钮时，增加一个全局loading，在结果返回之前，禁止用户点击，这是一个最简单的方法。但是由于我们同时有好几个请求，所以不太好去做一个统一控制，当然，也可以使用promise.all或者promise.allsette来解决。

**方案二：** 增加防抖函数，从而达到在规定的时间内，无论用户请求多少次，都只请求一次的效果。但是这也有一个缺点，就是这个时间不太好确定，如果短了，达不到防抖的效果；时间过长了，又会有一段长时间间隔没有请求。

**方案三：** 就是在axios的请求拦截器里面判断是否为同一请求，如果是，则取消请求。所以我的做法是，在请求拦截器中根据请求的方法名、参数、url、以及页面的hash值充当请求的key，然后定义一个set类型的全局变量pendingRequest用于存储请求的key，如果判断这个集合中已经存在了，就直接reject掉，否则继续请求。最后等请求完成时，在响应拦截器中删除对应的key


# 用户直接点击需登录的链接，页面数据空白

## 背景
我们门店的概念，而且只有门店的店员才能看到门店内的数据，当门店店员将这个链接直接copy分享给非门店店员用户时，这个非门店店员的用户点击链接，进去后会发现所有的数据都是空的，这其实没有问题，因为这个用户并不是门店店员吗？但是，这其实又是一个体验性的问题，就是，如果我没有权限查看这些门店数据，就不应该让我进入到这个页面，而不是说进入的是一个没有数据的门店页面，而且没有一点提示，或者显示一个空页面。

## 解决方案
这其实是一个权限校验的问题。我当时提出的方案是，基于axios封装一个请求接口组件。首先，我和后端约定某个特定的code代表的含义，比如code=0，代表正常；code=-1，代表没有权限；code=-2，代表没有登录等等，然后响应拦截器里面根据code值做出相应的跳转，如果没有权限，我们会跳到一个页面，就显示一张图片和一段提示用户没有查看权限的文字，然后，如果没有登录，就直接跳到登录页。除此之外，我还向外暴露了一些参数
...
## 亮点
## 使用媒体查询适配不同屏幕尺寸
pc端和移动端的布局是不一样的，所以需要使用媒体查询，根据屏幕尺寸的不同，渲染不同的布局。另外使用grid和flex布局实现响应式布局。

使用以下代码实现实现当元素数小于等于四个时，在一行平分展示，大于四个时换行展示
```css
.grid-container {
  display: grid;
  gap: 16px;
...
用户反馈无法进入门店直播数据查看自己的数据，收到反馈后我首先去现网查看，发现是正常的。然后我变身化作用户帐号去他们店铺查看，发现也是正常的。这就排出了该店铺在灰度环境的情况。再次向管家确认，收到商家依然没办法进入的答复。然后我找后端查看日志，发现这个用户Id没有请求接口，因为我们这个直播数据的页面是使用一个外链的方式嵌入到门店的项目当中的。于是怀疑可能是没有进入到我们这个页面。不过我进入到那个空白页，然后复制页面的url，发现确实也是进入到直播数据页面了。只是没有请求，页面也是空白的。这个时候就基本锁定是用户的设备问题了。然后通过管家得知，用户的设备是iphone6，比较老了，我上网查了一下，iphone6的系统最高只能到ios12，而我们里面有许多新的api是不支持旧版本的，比如`replaceAll`，这个api最低支持ios13+的，然后我就在`.browserlistrc`文件中把`babel`编辑的目标浏览器做了个降级。但是还是不行。后面通过从测试同学那借来测试机，加上`vconsole`打印，发现iphone6机型已进入页面就会报一个`chunkError`的问题。通过debugger发现我们之前同事一个本该从`vant`当中倒入的`list`组件是通过本地文件导入的，而本地的那个文件是一个`vant`经过编译后的文件。
至此，可以确定原因就是之前的同事讲`vant`编译成一个适用较高浏览器的文件存到本地，然后通过引入本地的这个编译后的包使用组件。当用户浏览器版本较高时，是不会出现问题的，但是一旦用户浏览器版本过低，就会出现api不支持的情况。

最终的解决方案就是删除本地的这个编译后的文件，然后按需导入`vant`

# 精度问题
是一个电商类的商家反馈他在给商品设置价格的时候，设置成18.9的时候，价格会变为18.89。我当时上去测试店铺去设置价格，发现没有复显，但是商品提供的录屏确实是有问题的。然后我把价格设置成他视频中的价格，也就是18.9，发现确实会变成18.89。

就是非常奇怪，有的价格设置是正常的，有的就会异常。所以我当时通过在chrome控制台打断点，一步一步执行，最终找到问题了。是因为我们前端展示的价格是以元为单位，后端是以分为单位，所以在设置价格后会乘以100，最终导致精度问题。这就和js中0.1 + 0.2 不等于0.3 一样。都是由于js使用64位双精度浮点数表示数字，二进制无法精确表示某些十进制小数，导致某些小数运算时存在精度丢失问题。

解决方案有两种，第一种就是先通过`toFixed`方法，因为`toFixed`方法将数值进行四舍五入并转换为字符串输出。但是这样会有个问题，就是`toFixed` 需要穿了一个参数确定保留的精度，到底保留多少位小数不太好确定，当然电商类的一般是保留小数点后两位，所以可以设置成2

第二种方案，就是借用一些第三方库，我当时用的就是`decimal.js`。

# 出海建站

实现一个店铺装修页面，商家可以通过简单的点击新增组件便可完成店铺首页的新建和修改。

1、总体流程：用户点击选择新增的组件，前端根据组件类型先初始化一个组件数据，并把这个组件选择在预览区，用户修改组件数据时，预览区实时渲染，保存后，将数据保存到后端，后端拿到json数据后，保存到数据库，在c端时，返回给到前端，前端根据数据渲染。

2、使用`vue-draggable-plus`实现允许客户拖动组件，改变组件顺序

3、通过`pinia`维护一个`historyStack`栈，控制组件编辑的层级，点击组件时push一个对象到栈中，对象属性包含：组件名称、组件信息以及index等信息。组件编辑区域通过获取`historyStack`中最后一个元素作为数据来源。删除组件时，从`historyStack`中删除该元素，并且通过`index`从组件列表中删除组件。

4、新增主横幅、轮播图、视频、商品列表等六个组件供用户选择

5、轮播图支持适应首图比例，按固定比例(5:2、16:9)、铺满全屏形式。通过监听用户选择的样式，动态计算轮播图的高度，
* 适应首图比例。通过以下方案实现：
```js
// 在 methods 中
methods: {
  loadImageSilently() {
    if (this.componentData.container_style !== 0) return;
    
    const img = new Image();
    img.onload = () => {
      this.imageLoadFn(); // 你的加载完成回调
    };
    img.onerror = (e) => {
      console.error('图片加载失败', e);
    };
    img.src = this.componentData.sub_list[0].img_url;
  }
}

// 在 mounted 或适当时候调用
mounted() {
  this.loadImageSilently();
}
```
这样实现当加载第一张图片，然后获取图片的宽度和高度，算出图片比例，然后使用`vueuse`中的`useElementSize`获取轮播图宽度，并根据图片比例计算高度，并设置到轮播图上。
除了上面的方案，也可以以使用css将图片设置为隐藏，来获取图片的宽度和高度，然后再计算比例。

* 按固定比例时，使用`vueuse`中的`useElementSize`获取轮播图宽度，然后根据比例计算高度，并设置到轮播图上。
* 铺满全屏时，获取`window.innerHeight`然后减去顶部导航栏高度，然后设置到轮播图上。
